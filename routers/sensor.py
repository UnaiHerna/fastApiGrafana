...
from db.models import SensorDatos, Sensor, Variable, Equipo
...
async def read_datos_sensor_by_variable(db, variable: str, start_date=None, end_date=None):
...
async def read_datos_sensor_by_equipo(db, equipo: str, start_date=None, end_date=None):
...
async def read_datos_sensor_multiple_by_variable(db, variables: str, start_date=None, end_date=None):
    variable_list = variables.split(',')
    all_data = {}
    for variable in variable_list:
        data = await read_datos_sensor_by_variable(db, variable, start_date, end_date)
        all_data[variable] = data
    return all_data


async def read_datos_sensor_multiple_by_equipos(db, equipos: str, start_date=None, end_date=None):
    equipo_list = equipos.split(',')
    all_data = {}
    for equipo in equipo_list:
        data = await read_datos_sensor_by_equipo(db, equipo, start_date, end_date)
        all_data[equipo] = data
    return all_data


async def read_datos_sensor_variable_by_equipo(db, variable: str, equipo: str, start_date=None, end_date=None):
...
@router.get("/")
async def datos_condicionales_sensor(
    ...
    if variable and not equipo and not variables and not equipos:
        return await read_datos_sensor_by_variable(db, variable, start_date, end_date)
    elif equipo and not variable and not variables and not equipos:
        return await read_datos_sensor_by_equipo(db, equipo, start_date, end_date)
    elif variables and not variable and not equipo and not equipos:
        return await read_datos_sensor_multiple_by_variable(db, variables, start_date, end_date)
    elif equipos and not variable and not variables and not equipo:
        return await read_datos_sensor_multiple_by_equipos(db, equipos, start_date, end_date)
    elif variable and equipo and not variables and not equipos:
        return await read_datos_sensor_variable_by_equipo(db, variable, equipo, start_date, end_date)
